/****************************************************************************
** $Id: qt/src/network/qsocketdevice_unix.cpp   2.3.10   edited 2005-01-24 $
**
...
// Darwin's still using int's... 
#    define SOCKLEN_T int
#  elif defined(_OS_FREEBSD_) && __FreeBSD_version < 400000
// FreeBSD up to 3.5.1
#    define SOCKLEN_T int
#  else
#    define SOCKLEN_T socklen_t
...
void QSocketDevice::close()
{
    ...
    ::close( fd );
#if defined(QSOCKETDEVICE_DEBUG)
    qDebug( "QSocketDevice::close: Closed socket %d", fd );
#endif
    fd = -1;
