/****************************************************************************
** $Id: qt/src/widgets/qcheckbox.cpp   2.3.10   edited 2005-01-24 $
**
...
static int extraWidth( const QWidget *w )
{
    if ( w->style().guiStyle() == Qt::MotifStyle )
	return 8;
    else
	return w->style().pixelMetric(QStyle::CheckBoxGap);
}
...
QSize QCheckBox::sizeHint() const
{
	...
	+ (text().isEmpty() ? 0 : 4 + extraWidth(this)),
...
void QCheckBox::drawButtonLabel( QPainter *p )
{
	...
	QSize sz = style().indicatorSize();
	br.setLeft( QMAX(br.left()-3, sz.width()+2) );
	br.setRight( QMIN(br.right()+2, width()) );
	br.setTop( QMAX(br.top()-2, 1) );
	br.setBottom( QMIN(br.bottom()+2, height()-2) );
