/**********************************************************************
** $Id: qt/src/widgets/qtableview.cpp   2.3.10   edited 2005-01-24 $
**
...
void QTableView::paintEvent( QPaintEvent *e )
{
	...
	if ( inherits( "QMultiLineEdit" ) ){
		...
		QPen savePen = paint.pen();
#ifdef QT_KEYPAD_MODE
		if( qt_modalEditingEnabled )
		    paint.setPen(QPen(colorGroup().background(), frameWidth()+4));
		else
#endif
		    paint.setPen(QPen(colorGroup().base(), frameWidth()+4));
		paint.drawRect(0, 0, width(), height());
		paint.setPen(savePen);
	    }
	}
...
#ifdef QT_KEYPAD_MODE
    if (qt_modalEditingEnabled && isModalEditing()){
	paint.fillRect(maxWX+1+margin(), 1, width(), height(), colorGroup().background()); // fill gap around scrollbars 
	paint.fillRect(lineWidth(), maxWY+1+margin(), width()-2*lineWidth(), height() - maxWY, colorGroup().background());  
    }
#endif
...
#ifdef QT_KEYPAD_MODE
	    if ( qt_modalEditingEnabled && !isModalEditing() )
		paint.fillRect( updateR, g.background() );
	    else
#endif
	    paint.fillRect( updateR, g.base() );
	} else {
	    paint.eraseRect( updateR );
	}
	return;
...
#ifdef QT_KEYPAD_MODE
	    if ( qt_modalEditingEnabled && !isModalEditing() )
		paint.fillRect( r.intersect( updateR ), g.background() );
	    else
#endif
...
{
    return width() - 1 - margin() -  frameWidth()
...
{
    return height() - 1 - margin() - frameWidth() 
...
void QTableView::updateScrollBars( uint f )
{
	...
	if ( sbDirty & horGeometry )
	    hScrollBar->setGeometry( frameWidth(), height() - HSBEXT - frameWidth(),
                                     viewWidth(),
                                   HSBEXT);
	...
	if ( sbDirty & verGeometry )
	    vScrollBar->setGeometry( width() - VSBEXT - frameWidth(), frameWidth(),
                                     VSBEXT,
                                     viewHeight());
	...
	cornerSquare->move( maxViewX() + 1 + margin(),
			    maxViewY() + 1 + margin());
