/****************************************************************************
** $Id: qt/src/kernel/qwidget.cpp   2.3.10   edited 2005-01-24 $
**
...
static QWidget *editing_widget=0;
bool qt_modalEditingEnabled = TRUE;
#endif
...
QWidget::~QWidget()
{
    ...
    if ( QApplication::active_window == this )
...
#ifdef QT_KEYPAD_MODE
class QFocusDataAccessor : public QWidget
{
public:
    QFocusData *getFocusData() {
	return focusData();
    }
};

bool qt_singleFocusWidget(const QWidget *in)
{
    bool single = TRUE;
    QFocusData *fd = ((QFocusDataAccessor*)in)->getFocusData();
    QWidget *start = fd->home();
    QWidget *w = start;
    while (w) {
	if ( w != in && (w->focusPolicy() & QWidget::TabFocus) == QWidget::TabFocus &&
...
/*!
    \Internal
*/
bool QWidget::isModalEditing() const
{
    return editing_widget == this;
}

/*!
    \Internal
*/
void QWidget::setModalEditing(bool on)
{
    if ( editing_widget && editing_widget != this )
	editing_widget->setModalEditing( FALSE );
    
    if ( (!on && !editing_widget) || (on && editing_widget == this) ){
	update();
	return;
    }

    if (!on && editing_widget == this) {
	editing_widget = 0;
	QEvent event(QEvent::LeaveModalEdit);
	QApplication::sendEvent( this, &event );
    } else if ( on ) {
	editing_widget = this;
	QEvent event(QEvent::EnterModalEdit);
	QApplication::sendEvent( this, &event );
    }
    update();
}

QWidget* QWidget::editingWidget() const
{
...
void QWidget::setFocus()
{
	...
	QInputContext::reset(this);
#endif
	if ( prev != this ) {
	    if ( prev ) {
#ifdef QT_KEYPAD_MODE
		if( qt_modalEditingEnabled ) {
		    if ( prev->isModalEditing() )
			prev->setModalEditing( FALSE );
		}
#endif
	    ...
	    if( qt_modalEditingEnabled ) {
		if ( editing_widget != this &&
		     (focusPolicy() & QWidget::TabFocus) == QWidget::TabFocus &&
		     qt_singleFocusWidget(this) ) {
	...
	case QEvent::KeyPress: {
		...
		if( qt_modalEditingEnabled ) {
			if ( !k->isAccepted() ) {
			    ...
			    qt_singleFocusWidget(this) && !w->isModalEditing() &&

				    (k->key() == Key_Back || k->key() == Key_No)
			    ) {
			    // Always propagate Key_Back presses to top level if widget
			    // isn't in modal editing state, even if the widget
			    // accepts it
			    k->ignore();
			}
		}
		...
		// default to accept. widgets should explicitly ignore if they 
		// don't handle the event
		k->accept();
	    }
#ifdef QT_KEYPAD_MODE
	    // available on all modes
	    if (w && w->isTopLevel() && !k->isAccepted() &&
		(k->key() == Key_Back || k->key() == Key_No) &&
		!w->testWFlags(WStyle_Dialog|WStyle_Customize|WType_Popup|WType_Desktop)) {
...
# if !defined(QT_NO_QWS_MANAGER) && defined(_WS_QWS_)
...
void QWidget::setBackgroundOrigin( BackgroundOrigin origin )
{
    ...
    clearWState( WState_TranslateBackground1 );

    if ( origin == WindowOrigin || origin == AncestorOrigin )
	setWState( WState_TranslateBackground1 );
    if ( origin == ParentOrigin || origin == AncestorOrigin )
...
QWidget::BackgroundOrigin QWidget::backgroundOrigin() const
{
	...
	if (testWState( WState_TranslateBackground1 ) )
	    return AncestorOrigin;
	else
	    return ParentOrigin;
    } else if (testWState( WState_TranslateBackground1 ) ) {
	return WindowOrigin;
    } else {
	return WidgetOrigin;
    }
...
QPoint qt_backgroundOffset(const QWidget *w)
{
    if ( w->testWState(Qt::WState_GlobalBrushOrigin) && w->backgroundMode() != QWidget::FixedPixmap )
	return w->mapToGlobal(QPoint(0, 0));

    if (!w->isTopLevel()) {
	switch(w->backgroundOrigin()) {
		...
		{
		    const QWidget *topl = w->topLevelWidget();
		    return w->mapTo((QWidget *)topl, QPoint(0, 0) );
		}
	    case QWidget::AncestorOrigin:
		{
		    const QWidget *topl = w;
		    bool ancestorIsWindowOrigin = FALSE;
		    while(topl && !topl->isTopLevel())
		    {
