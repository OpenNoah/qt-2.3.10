/*****************************************************************************
** $Id: qt/src/kernel/qgfxvnc_qws.cpp   2.3.10   edited 2005-01-24 $
**
...
#ifdef QT_VNC_VFB
# define VNCSCREEN_BASE	QVFbScreen
#else
...
QVNCServer::QVNCServer()
    : QServerSocket( 5900, (int) 0, (QObject *) 0 )
{
...
bool QVNCScreen::connect( const QString &displaySpec )
{
	...
	lstep = ( dw * d + 7 ) / 8;
#if !defined(_OS_QNX6_) && !defined(QT_VNC_VFB)
	dataoffset = 0;
	canaccel = FALSE;
	optype = &dummy_optype;
	lastop = &dummy_lastop;
...
bool QVNCScreen::initDevice()
{
    ...
    vncServer = new QVNCServer( displayId );
