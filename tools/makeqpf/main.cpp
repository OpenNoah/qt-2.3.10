...
#ifdef _WS_QWS_
	int mixer = 0;
	if ( from == 0 && to == 65535 ) mixer = 64;
	else if ( from == 0 && to == 255 ) mixer = 32;
	else if ( from == 0 && to == 127 ) mixer = 16;
	for (int i=from; i<=to; i++) {
	    int ch = i^mixer;
	    if ( memorymanager->inFont((QMemoryManager::FontID)font.handle(),QChar(ch))) {
		memorymanager->lockGlyph((QMemoryManager::FontID)font.handle(),QChar(ch));
		memorymanager->unlockGlyph((QMemoryManager::FontID)font.handle(),QChar(ch));
	    }
	}
#endif
    }
...
public:
	...
	readFontDir(fontdir);
        ...
        QPopupMenu *quitMenu = new QPopupMenu( this );
        quitMenu->insertItem( "Quit  MakeQPF &q", this , SLOT(close()), CTRL+Key_Q);
	quitMenu->hide();
