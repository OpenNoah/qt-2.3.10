...
QString Uic::trcall( const QString& txt, const QDomNode& node )
{
    return trcall(txt,getComment(node));
}

QString Uic::trcall( const QString& txt, const QString& com )
{
    QString r = trmacro + "( " + fixString(txt);
    if ( !com.isEmpty() )
	r += ", " + fixString(com);
    return r + " )";
}
...
QString Uic::createObjectImpl( const QDomElement &e, const QString& parentClass, const QString& parent, const QString& layout )
{
	...
	for ( n = e.firstChild().toElement(); !n.isNull(); n = n.nextSibling().toElement() ) {
		...
		out << indent << objName << "->insertTab( " << page << ", " << trcall( label, comment ) << " );" << endl;
	    }
	}
     } else { // standard widgets
	for ( n = e.firstChild().toElement(); !n.isNull(); n = n.nextSibling().toElement() ) {
...
QString Uic::createListBoxItemImpl( const QDomElement &e, const QString &parent )
{
    ...
    return pix.isEmpty() ?
      parent + "->insertItem( " + trcall( txt, com ) + " );":
      parent + "->insertItem( " + pix + ", " + trcall( txt, com ) + " );";
}
				     ...
				     const QString &parentItem )
{
	...
	if ( !textes[ i ].isEmpty() )
	    s += indent + item + "->setText( " + QString::number( i ) + ", " + trcall( textes[ i ], coms[ i ] ) + " );\n";
...
QString Uic::setObjectProperty( const QString& objClass, const QString& obj, const QString &prop, const QDomElement &e, bool stdset )
{
	...
	if ( prop == "toolTip" ) {
	    if ( !obj.isEmpty() )
		out << indent << "QToolTip::add(  " << obj << ", " + trcall( e.firstChild().toText().data(), getComment(e.parentNode()) ) << " );" << endl;
	    else
		out << indent << "QToolTip::add(  this, " + trcall( e.firstChild().toText().data(), getComment(e.parentNode()) ) << " );" << endl;
	} else if ( prop == "whatsThis" ) {
	    if ( !obj.isEmpty() )
		out << indent << "QWhatsThis::add(  " << obj << ", " << trcall( e.firstChild().toText().data(), getComment(e.parentNode()) ) << " );" << endl;
	    else
		out << indent << "QWhatsThis::add(  this, " << trcall( e.firstChild().toText().data(), getComment(e.parentNode()) ) << " );" << endl;
	} else {
	    v = trcall(e.firstChild().toText().data(), getComment(e.parentNode()));
	}
