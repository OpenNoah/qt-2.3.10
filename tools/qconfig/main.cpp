...
public:
    ...
    void setFeatureEnabled(bool y) { setOn(y); }
    bool isFeatureEnabled() const { return isOn(); }
...
*/
	...
	if ( isAvailable() ) {
	    ...
	    QColorGroup c = cg;
	    c.setColor(QColorGroup::Text,
		!isFeatureEnabled() ? lightGray : red);
    ...
    QString info() const
    {
	return "<h2>"+label+" <small>["+id+"]</small></h2>"+doc;
    }
...
Main::Main()
{
    ...
    connect(lv, SIGNAL(pressed(QListViewItem*)),
	    this, SLOT(updateAvailabilities(QListViewItem*)));
...
void Main::save()
{
    QString filename = QFileDialog::getSaveFileName("qconfig.h",QString::null,this);
...
void Main::loadFeatures(const QString& filename)
{
	...
	if ( line.length() <= 1 ) {
	    ...
	    feature = lab = sec = doc = QString::null;
...
qDebug("load %s",filename.latin1());
    ...
    for (QStringList::Iterator ch = choices.begin(); ch != choices.end(); ++ch) {
	item[*ch]->setFeatureEnabled(TRUE);
    }
    do {
	QString line = s.readLine();
	QStringList token = QStringList::split(QChar(' '),line);
	if ( token[0] == "#define" && token[1].left(6) == "QT_NO_" ) {
	    QString id = token[1].mid(6);
	    ChoiceItem* i = item[id];
	    if ( i )
		i->setFeatureEnabled(FALSE);
	    else
...
void Main::updateAvailabilities(QListViewItem* i)
{
    for (QStringList::Iterator ch = choices.begin(); ch != choices.end(); ++ch) {
	updateAvailability(item[*ch]);
    }
    if ( i )
	showInfo(i);
}
...
{
    if ( i->parent() ) {
	ChoiceItem* c=(ChoiceItem*)i;
	QStringList deps = dependencies[c->id];
	if ( !deps.isEmpty() ) {
	    bool av = TRUE;
		...
		if ( d ) {
		    ...
		    if ( !d->isAvailable() || !d->isFeatureEnabled() )
			av = FALSE;
		}
	    }
	    c->setAvailable(av);
...
void Main::showInfo(QListViewItem* i)
{
	...
	if ( !deps.isEmpty() ) {
		...
		if ( d ) {
		    QString color;
		    if ( d->isAvailable() ) {
			if ( !d->isFeatureEnabled() )
			    color = "red";
		    } else {
			color = "#a0a0a0";
		    }
		    i += "<li>";
		    if ( !color.isNull() ) i += "<font color="+color+">";
		    i += "<a href=id://"+d->id+">"+d->label+"</a>";
		    if ( !color.isNull() ) i += "</font>";
		}
	    }
	    i += "</ul>";
	}
	...
	if ( !rdeps.isEmpty() ) {
		...
		ChoiceItem* d = item[*it];
		if ( d ) {
		    bool ok = choice->isFeatureEnabled() || !d->isFeatureEnabled();
		    i += "<li>";
		    if ( !ok ) i += "<font color=red>";
		    i += "<a href=id://"+d->id+">"+d->label+"</a>";
		    if ( !ok ) i += "</font>";
		}
	    }
...
{
qDebug("Main::selectId(%s)",id.latin1());
...
qDebug("setSource(%s)",name.latin1());
    if ( name.left(5) == "id://" ) {
	emit idClicked(name.mid(5)); // skip trailing "/" too
    } else {
